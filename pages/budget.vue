    ...
    <div class="max-w-6xl mx-auto">
      ...
      <div class="card p-6 mb-8">
        <div class="flex items-center justify-between mb-4">
          <h2 class="text-xl font-medium text-neutral-900 dark:text-neutral-50">Budget Total - {{ currentPeriodLabel }}</h2>
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
          ...
          <div class="card bg-white dark:bg-neutral-800 p-4 shadow-sm">
            ...
            <p class="text-2xl font-bold" :class="totalExpenses > totalBudget ? 'text-red-600 dark:text-red-400' : 'text-neutral-900 dark:text-neutral-50'">{{ formatCurrency(totalExpenses) }}</p>
          </div>
          
          <div class="card bg-white dark:bg-neutral-800 p-4 shadow-sm">
            <h3 class="text-sm font-medium text-neutral-600 dark:text-neutral-400 mb-1">Restant</h3>
            <p class="text-2xl font-bold" :class="totalBudget - totalExpenses < 0 ? 'text-red-600 dark:text-red-400' : 'text-green-600 dark:text-green-400'">{{ formatCurrency(totalBudget - totalExpenses) }}</p>
          </div>
        </div>
        
        <div class="mb-3">
          <div class="flex items-center justify-between mb-2">
            <span class="text-sm font-medium text-neutral-700 dark:text-neutral-300">Progression : {{ Math.round((totalExpenses / totalBudget) * 100) || 0 }}%</span>
            <span class="text-sm text-neutral-600 dark:text-neutral-400">{{ formatCurrency(totalExpenses) }} / {{ formatCurrency(totalBudget) }}</span>
          </div>
      ...
      <div class="card p-6">
        <div class="flex items-center justify-between mb-6">
          ...
          <div class="flex items-center space-x-2">
            ...
            <select v-model="currentPeriod" class="px-3 py-1 bg-white dark:bg-neutral-800 border border-neutral-300 dark:border-neutral-700 rounded text-sm">
        ...
        <div v-else class="space-y-6">
          ...
          <div v-for="budget in sortedBudgets" :key="budget.id" class="bg-white dark:bg-neutral-800 p-4 rounded-lg">
            <div class="flex items-center justify-between mb-2">
              ...
              <div class="flex items-center space-x-2">
                <button @click="editBudget(budget)" class="text-neutral-500 hover:text-primary-600 dark:hover:text-primary-400">
                  <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" />
                  </svg>
                </button>
                <button @click="confirmDeleteBudget(budget)" class="text-neutral-500 hover:text-red-600 dark:hover:text-red-400">
                  <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            ...
            <div class="mb-2 flex justify-between">
              <span class="text-neutral-700 dark:text-neutral-300">
                {{ formatCurrency(getCategoryExpenses(budget.category)) }} dépensés sur {{ formatCurrency(budget.amount) }}
              </span>
              <span 
                class="font-medium"
              ...
              >
                {{ getCategoryExpenses(budget.category) > budget.amount ? 'Dépassé' : 'Restant' }}: {{ formatCurrency(Math.abs(budget.amount - getCategoryExpenses(budget.category))) }}
              </span>
            </div>
...
import { EXPENSE_CATEGORIES } from '@/server/utils/models';
